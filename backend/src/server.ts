...
function calculateSummary(airdrops: any[]) {
  const now = new Date();
  
  let totalAirdrops = airdrops.length;
  let completed = 0;
  let pending = 0;
  let nextDeadline = null as string | null;
...
app.post("/telegram-webhook", async (req: Request, res: Response) => {
  try {
    ...
    const sendBotMessage = async (textToSend: string) => {
...
app.post("/auth/request-otp", async (req: Request, res: Response) => {
  try {
...
app.post("/auth/otp/generate", authenticateToken, async (req: Request, res: Response) => {
  try {
...
// 6. GET /api/airdrops
app.get("/api/airdrops", authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
...
// 7. POST /api/airdrops
app.post("/api/airdrops", authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).userId;
    ...
    if (body.length > 0) {
      const rowsToInsert = body.map((a: any) => ({
